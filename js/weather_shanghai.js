$(function(){
	$.getJSON('http://api.openweathermap.org/data/2.5/forecast?id=1796236&appid=b9a622a92a130dca23f9755d97503990&units=metric',function(data){
		...
		$('.sHumi').prepend($cHumi);
		$('.sSpd').append($cSpd);
		$('.sDeg').append(whatdegE);
		$('.smonth').prepend(month);
		$('.sdate').prepend(date);
		$('.swhatday').prepend(whatday);
		$('.sicon').append('<img src="http://openweathermap.org/img/wn/'+$wIcon+'@2x.png" />');
	});
});